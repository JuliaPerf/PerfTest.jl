...
if isdefined(Test, :TestCounts)
    ...
    function Test.get_test_counts(ts::PerfTestSet)
        ...
        duration = ""
        return Test.TestCounts(true, passes, fails, errors, broken,
            c_passes, c_fails, c_errors, c_broken, duration)
    end
    ...
    function Test.finish(ts::PerfTestSet)
        ...
        for (test_name, test_result) in ts.test_results
            if let c = Test.get_test_counts(ts)
                c.fails + c.errors
            end > 0 || Configuration.CONFIG["general"]["verbose"] > 0
    ...
    function Test.finish(ts::PerfTestSet)
        ...
        # Print failed tests on the current level (or everything if verbose)
        for (test_name, test_result) in ts.test_results
            if sum(Test.get_test_counts(ts)[2:3]) > 0 || Configuration.CONFIG["general"]["verbose"] > 0
                for parent in ts.parent_chain
                    print("IN: $(parent) ")
                end
                print(
    ...
    function Test.print_test_results(ts::PerfTestSet)
        ...
        passes, fails, errors, _, cp, cf, ce, _ = Test.get_test_counts(ts)
        print("Aggregate Results: $(passes + cp) PASSED, $(fails + cf) FAILED, $(errors+ce) ERRORS\n")
        err = get_test_errors(ts)
        for _error in err
            println("ERROR:")
...
function testsSucceeded(ts::PerfTestSet)
    for t in ts.results
        if isa(t, Fail) || isa(t, Error)
            return false
