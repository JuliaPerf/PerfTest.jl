...
using CountFlops
path = "./.perftests/t5-recursive.jl_PERFORMANCE.JLD2"
nofile = true
if isfile(path)
    nofile = false
...
let
    ...
    peakbandwidth = bench_data.multi.maximum * 1.0e6
    global ((_PRFT_GLOBAL[:machine])[:empirical])[:peakmemBW] = peakbandwidth
end
_PRFT_LOCAL_SUITE = BenchmarkGroup()
_PRFT_LOCAL_ADDITIONAL = Dict()
_PRFT_LOCAL_SUITE["RECURSIVE"] = BenchmarkGroup()
_PRFT_LOCAL_ADDITIONAL["RECURSIVE"] = Dict()
let _PRFT_LOCAL_SUITE = _PRFT_LOCAL_SUITE["RECURSIVE"], _PRFT_LOCAL_ADDITIONAL = _PRFT_LOCAL_ADDITIONAL["RECURSIVE"]
    using Test
    using PerfTest
    ...
    _PRFT_GLOBAL = Dict{Symbol, Any}()
    using CountFlops
    path = "./.perftests/t4-raw.jl_PERFORMANCE.JLD2"
    nofile = true
    if isfile(path)
        nofile = false
        _PRFT_GLOBAL[:datafile] = PerfTest.openDataFile(path)
    ...
    let _PRFT_LOCAL_SUITE = _PRFT_LOCAL_SUITE["FIRST LEVEL"], _PRFT_LOCAL_ADDITIONAL = _PRFT_LOCAL_ADDITIONAL["FIRST LEVEL"]
        ...
        let _PRFT_LOCAL_SUITE = _PRFT_LOCAL_SUITE["SECOND LEVEL"], _PRFT_LOCAL_ADDITIONAL = _PRFT_LOCAL_ADDITIONAL["SECOND LEVEL"]
            PerfTest.CustomMetric[PerfTest.CustomMetric("Time elapsed median", "s", :((_PRFT_LOCAL[:primitives])[:median_time]), Symbol("Time elapsed median"), true), PerfTest.CustomMetric("Operational intensity", "Flop/Byte", :((_PRFT_LOCAL[:primitives])[:autoflop] / 1), :opInt, false), PerfTest.CustomMetric("Attained Flops", "FLOP/s", :((_PRFT_LOCAL[:primitives])[:autoflop] / (_PRFT_LOCAL[:primitives])[:median_time]), :attainedFLOPS, false)]
            PerfTest.MethodologyParameters[PerfTest.MethodologyParameters(:roofline, "Roofline Model", true, Dict{Symbol, Any}(Symbol("") => quote
    #= none:23 =#
    :autoflop / 1
end, :target_ratio => 0.05, :test_flop => true, :actual_flops => :(:autoflop), :test_opint => false))]
            x = begin
                    ...
                    @testset "SECOND LEVEL" showtiming = false begin
                            ...
                            let _PRFT_LOCAL = _PRFT_LOCAL["SECOND LEVEL"]
                                ...
                                let _PRFT_LOCAL = _PRFT_LOCAL["Test 1"]
                                    ...
                                    (_PRFT_LOCAL[:primitives])[:median_time] = (median(_PRFT_LOCAL[:suite])).time / 1.0e9
                                    (_PRFT_LOCAL[:primitives])[:min_time] = (minimum(_PRFT_LOCAL[:suite])).time / 1.0e9
                                    ...
                                    (_PRFT_LOCAL[:metrics])[Symbol("Time elapsed median")] = Metric_Result("Time elapsed median", "s", (_PRFT_LOCAL[:primitives])[:median_time], true)
                                    (_PRFT_LOCAL[:metrics])[:opInt] = Metric_Result("Operational intensity", "Flop/Byte", (_PRFT_LOCAL[:primitives])[:autoflop] / 1, false)
                                    (_PRFT_LOCAL[:metrics])[:attainedFLOPS] = Metric_Result("Attained Flops", "FLOP/s", (_PRFT_LOCAL[:primitives])[:autoflop] / (_PRFT_LOCAL[:primitives])[:median_time], false)
                                    let
                                        ...
                                        result_flop_ratio = Metric_Result(name = "Attained FLOP/S by expected FLOP/S", units = "%", value = (flop_s / roof(opint)) * 100)
                                        ...
                                        aux_mem = Metric_Result(name = "Peak empirical bandwidth", units = "B/s", value = ((_PRFT_GLOBAL[:machine])[:empirical])[:peakmemBW])
                                        aux_flops = Metric_Result(name = "Peak empirical flops", units = "FLOP/s", value = ((_PRFT_GLOBAL[:machine])[:empirical])[:peakflops])
